...
def assign_shift_user(
    db: Session,
    id: int,
    users: list[int],
    is_commit: bool = True
):
    stmt = sql_delete(user_shift).where(user_shift.c.shift_id == id)
    db.execute(stmt)

    if users != []:
        mapping = [{"user_id": val, "shift_id": id} for val in users]
        stmt = insert(user_shift).values(mapping)
        db.execute(stmt)
