...
class TestAssignShiftUser(IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        alembic_args = ["-c", "alembic.ini", "upgrade", "head"]
        alembic.config.main(argv=alembic_args)
        self.db : Session = factory_session()
    ...
    async def test_assign_shift_to_user(self):
        list_role = [
        ...
        initial_shift(db=self.db, is_commit=False)
        self.db.commit()
        get_shift = self.db.query(Shift).filter(Shift.nama_shift == "Pagi").first()
        token = await generate_jwt_token_from_user(user=list_users[0])
        client = TestClient(app)
        data = {
            "user_id": [val.id for val in list_users]
        }

        # When
        response = client.post(
            f"/shift/{get_shift.id}/assign-user",
            headers={"Authorization": f"Bearer {token}"},
            json=data
        )
        ...
        output = {
            ...
            "jam_mulai": str(get_shift.jam_mulai),
            "jam_akhir": str(get_shift.jam_akhir),
            "users": [
                {
                    "id": val.id,
                    "email": val.email,
        ...
        self.maxDiff = None
        self.assertEqual(output, response.json())
        check_db = self.db.get(Shift, get_shift.id).shiftUser
        self.assertEqual(check_db, list_users)
